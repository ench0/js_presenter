...
var jamaahCalc = function(num, time, timenext){
  ...
  // console.log(timenext)
	...
	switch (jamaahMethodSetting) {
		...
		// case "fixed": var jamaahOffset = (moment().hour(jamaahOffsetSetting[0]).minute(jamaahOffsetSetting[1]).subtract(1, 'minute')).diff(time, 'minutes'); break;
...
var Prayer = function(now, num) {
  ...
  // this.time2 = moment({hour: timetable[this.now.month()+1][this.now.date()][num][0], minute: timetable[this.now.month()+1][this.now.date()][num][1]});
  ...
  // this.timenext = moment({hour: timetable[this.now.month()+1][this.now.date()][this.nextnum][0], minute: timetable[this.now.month()+1][this.now.date()][this.nextnum][1]});
  ...
  this.disp = this.time.format("HH:mm - DD");

  this.jamaahtime = this.time2.add(jamaahCalc(num, this.time2, this.timenext), 'minutes')
  this.jamaahdisp = this.jamaahtime.format("HH:mm - DD");
